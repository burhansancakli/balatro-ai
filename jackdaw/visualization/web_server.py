...
class WebServer:
    ...
    def start(self) -> None:
        threading.Thread(target=self._run_http, daemon=True, name="jackdaw-http").start()
        threading.Thread(target=self._run_ws,   daemon=True, name="jackdaw-ws").start()
        self._started.wait(timeout=8)
        print(f"\n  [jackdaw] Dashboard → http://{self._host}:{self._port}\n")
