...
Applies a player :data:`~engine.actions.Action` to the game state
and advances the phase.  Each handler validates legality, executes the
action, and transitions to the next phase as needed.

Usage::

    from emulator.engine.game import step
    from emulator.engine.actions import SelectBlind
...
def _handle_select_blind(gs: dict[str, Any]) -> dict[str, Any]:
    ...
    from emulator.engine.run_init import start_round
...
def _fire_new_blind_choice_tags(gs: dict[str, Any]) -> None:
    ...
    """
    from emulator.engine.tags import Tag
    ...
    for entry in awarded:
        ...
        if result.reroll_boss:
            from emulator.engine.blind import get_new_boss
...
def _open_tag_pack(gs: dict[str, Any], pack_key: str) -> None:
    ...
    """
    from emulator.engine.data.prototypes import BOOSTERS
    from emulator.engine.packs import generate_pack_cards
...
def _apply_tag_result(gs: dict[str, Any], result: Any) -> None:
    ...
    if result.create_jokers:
        from emulator.engine.card_factory import create_card
...
def _handle_skip_blind(gs: dict[str, Any]) -> dict[str, Any]:
    ...
    # ------------------------------------------------------------------
    from emulator.engine.jokers import GameSnapshot, JokerContext, calculate_joker
...
def _handle_play_hand(gs: dict[str, Any], indices: tuple[int, ...]) -> dict[str, Any]:
    ...
    # ------------------------------------------------------------------
    from emulator.engine.scoring import score_hand
    ...
    if (
        ...
        from emulator.engine.data.hands import HandType as _HT
...
def _handle_discard(gs: dict[str, Any], indices: tuple[int, ...]) -> dict[str, Any]:
    ...
    # ------------------------------------------------------------------
    from emulator.engine.jokers import JokerContext, calculate_joker
    ...
    for eff in pre_discard_effects:
        if eff.level_up:
            hand_levels = gs.get("hand_levels")
            if hand_levels is not None:
                from emulator.engine.hand_eval import evaluate_hand
    ...
    for card in discarded:
        ...
        if getattr(card, "seal", None) == "Purple":
            ...
            if len(consumables) < consumable_limit:
                from emulator.engine.card import Card as _Card
        ...
        for joker in jokers:
            ...
            if result:
                ...
                if result.level_up:
                    ...
                    if hl is not None:
                        from emulator.engine.hand_eval import evaluate_hand as _eval
...
def _build_discard_snapshot(gs: dict[str, Any], jokers: list) -> Any:
    ...
    from emulator.engine.jokers import GameSnapshot
...
def _handle_use_consumable(
    ...
    from emulator.engine.consumables import can_use_consumable
...
def _handle_redeem_voucher(gs: dict[str, Any], idx: int) -> dict[str, Any]:
    ...
    from emulator.engine.vouchers import apply_voucher
...
def _round_won(gs: dict[str, Any]) -> None:
    ...
    11. Phase → ROUND_EVAL
    """
    from emulator.engine.economy import calculate_round_earnings
    from emulator.engine.round_lifecycle import process_round_end_cards
    ...
    # ------------------------------------------------------------------
    from emulator.engine.jokers import GameSnapshot, on_end_of_round
    ...
    for c in hand:
        if getattr(c, "seal", None) == "Blue" and not getattr(c, "debuff", False):
            if len(consumables) < consumable_limit and hand_levels is not None:
                ...
                from emulator.engine.consumables import _PLANET_HAND
                ...
                for pk, ht in _PLANET_HAND.items():
                    if ht == most_played.value:
                        ...
                        break
                if planet_key:
                    from emulator.engine.card import Card as _BSCard
...
def _advance_ante(gs: dict[str, Any]) -> None:
    ...
    from emulator.engine.tags import assign_ante_blinds
    ...
    for mut in mutations:
        ...
        if "create" in mut:
            ...
            if ctype == "playing_card":
                ...
                consumables: list = gs.setdefault("consumables", [])
                from emulator.engine.card import Card as _Card2
...
def _use_consumable_card(
    ...
    """
    from emulator.engine.consumables import ConsumableContext, use_consumable
...
def _resolve_create_descriptors(gs: dict[str, Any], descriptors: list[dict[str, Any]]) -> None:
    ...
    Delegates to :func:`~engine.card_factory.resolve_create_descriptor`
    for actual card creation.
    """
    from emulator.engine.card_factory import resolve_create_descriptor
...
def _sync_played_hand_types(gs: dict[str, Any]) -> None:
    ...
    """
    from emulator.engine.hand_levels import HandLevels
...
def _reroll_shop_cards(gs: dict[str, Any]) -> None:
    ...
    """
    from emulator.engine.card_factory import create_card
    from emulator.engine.shop import select_shop_card_type
