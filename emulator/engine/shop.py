...
from emulator.engine.data.prototypes import BOOSTERS, CENTER_POOLS

if TYPE_CHECKING:
    from emulator.engine.card import Card
    from emulator.engine.card_area import CardArea
    from emulator.engine.rng import PseudoRandom
...
def select_shop_card_type(
    ...
    * Ghost Deck — sets ``spectral_rate`` at run start
    ...
    ----------
    rng:
        Live :class:`~engine.rng.PseudoRandom` instance.  Advances
...
def roll_illusion_modifiers(
    ...
    * Seal is not determined here — deferred to the post-creation hook pass.
    ...
    2. ``'illusion_enh_pick' + append + str(ante)`` — enhancement selection
       (seeded via :meth:`~engine.rng.PseudoRandom.seed` passed to
       :meth:`~engine.rng.PseudoRandom.element`) — consumed only
    ...
    ----------
    rng:
        Live :class:`~engine.rng.PseudoRandom` instance.
    ante:
        Current ante number.
    append:
...
def populate_shop(
    ...
    game_state:
        ...
        * All keys forwarded to
          :func:`~engine.card_factory.create_card` via
          *game_state*
    ...
    from emulator.engine.card_factory import create_card, create_voucher
...
def reroll_shop(
    ...
    6. **Repopulate** — fill slots up to ``shop['joker_max']`` (default 2)
       via :func:`~engine.shop.populate_shop` logic (calls
       :func:`select_shop_card_type` then
       :func:`~engine.card_factory.create_card`).
