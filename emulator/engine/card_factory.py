...
from emulator.engine.data.enums import Rank, Suit
    ...
    *,
    ...
    3. **Joker modifiers** (only when ``card.ability["set"] == "Joker"`` and
       *area* is ``"shop"`` or ``"pack"``):
       ...
       c. Edition roll via :func:`~engine.card_utils.poll_edition`.
    ...
    game_state:
        ...
        Pool-filtering keys (forwarded to :func:`~engine.pools.get_current_pool`):
    ...
    from emulator.engine.pools import check_soul_chance, pick_card_from_pool
...
def resolve_create_descriptor(
    ...
    handlers (:mod:`engine.jokers`) and consumable handlers
    (:mod:`engine.consumables`) and the actual card-creation
    machinery.
