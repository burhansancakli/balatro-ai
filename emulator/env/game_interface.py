...
from emulator.engine.actions import Action, GamePhase
...
class DirectAdapter:
    ...
    def step(self, action: Action) -> GameState:
        from emulator.engine.game import step as engine_step

        engine_step(self._gs, action)
        return _snapshot(self._gs)

    def get_legal_actions(self) -> list[Action]:
        from emulator.engine.actions import get_legal_actions as engine_legal
...
class BridgeAdapter:
    """Adapter that wraps a :class:`~bridge.backend.Backend`.
    ...
    def reset(
        ...
        *,
        ...
        from bridge.backend import DECK_FROM_BOT, STAKE_FROM_BOT
    ...
    def step(self, action: Action) -> GameState:
        from bridge.balatrobot_adapter import action_to_rpc
    ...
    def get_legal_actions(self) -> list[Action]:
        from bridge.backend import SimBackend
    ...
    def _build_gs(self) -> dict[str, Any]:
        ...
        from bridge.balatrobot_adapter import bot_state_to_game_state
